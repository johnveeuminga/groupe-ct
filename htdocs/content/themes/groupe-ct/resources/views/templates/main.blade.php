		...
		<div id="interstitiel" class="lity-hide" style="background-image: url('{{ themosis_assets() }}/images/bg/interstiel.jpg');">
			...
			<div class="btn-container">
				...
				@if (!empty(get_field('acquisition_2_cta_link', $acquisition->ID)))
					<a class="btn-bloc2 primary-btn" style="margin: 0 15px;" href="{{ get_field('acquisition_1_cta_link', $acquisition->ID) }}" @if(get_field('acquisition_2_cta_blank', $acquisition->ID))target="_blank" @endif>{!! get_field('acquisition_1_cta_label', $acquisition->ID) !!}</a>
				@endif
				@if (!empty(get_field('acquisition_2_cta_link', $acquisition->ID)))
